...
{
	SECTION("Magnitude of real number is number itself")
	{
		Complex real = 6.54;
...
TEST_CASE("Multiplication of complex numbers")
{
	...
	SECTION("Distributive property")
	{
		...
		REQUIRE(result1.Re() == Approx(result2.Re()));
		REQUIRE(result1.Im() == Approx(result2.Im()));
	}
}
...
TEST_CASE("Division of complex numbers")
{
	...
	SECTION("Assigning division complex number by real number")
	{
		...
		z /= Complex(-5, 3);
		REQUIRE(z.Re() == Approx(2));
		REQUIRE(z.Im() == Approx (-10));
	}
}
